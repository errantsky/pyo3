...
- Make `PyType::is_subclass`, `PyErr::is_instance` and `PyAny::is_instance` operate not on a type known at compile-time but a run-time type object.
  The old behavior is still available as `PyType::is_subclass_of`, `PyErr::is_instance_of` and `PyAny::is_instance_of`.  [#1985](https://github.com/PyO3/pyo3/pull/1985)
...
- Remove `PyType::is_instance`, which is unintuitive; instead of `typ.is_instance(obj)`, you should
  now use `obj.is_instance(typ)`. [#1985](https://github.com/PyO3/pyo3/pull/1985)
